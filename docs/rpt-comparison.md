...
_Fonte lato nostro: **il codice su `main`**, verificato riga per riga il 2026-08-06, riverificato dopo la wave di chiusura il 2026-08-07 e riallineato il 2026-08-08 dopo il codice risorsa e i tre seam dichiarati — **2576 test unitari** su 119 file (verdi anche a UTC+14 e UTC−8), 827 check smoke API. 46 tabelle, 20 migrazioni, 51 rotte, 7 adapter di integrazione. Non la roadmap, non la memoria di sessione (che su due punti si è rivelata già superata)._
**Dove siamo, al 2026-08-08.** PARI **26**, AVANTI **16**, PARZIALE **12**, MANCA **2** su 56 capacità.

Due letture, ed è giusto dare entrambe perché misurano cose diverse: **42/56 (75%)** è dove siamo alla pari o avanti; **54/56 (96%)** è dove esiste qualcosa — contando anche i PARZIALE, che qui significa quasi sempre «il seam è costruito e dichiarato non connesso», non «mezzo fatto».

Le **2 MANCA** residue sono la riga 2 (i18n IT/EN a runtime) e la riga 7 (deleghe + act-as). Nessuna delle due è bloccata da un terzo: sono lavoro nostro, non ancora fatto.
...
| 10 | inserimento per **Codice ID digitabile** (`COGNOM` + `NOM` + `00000` + progressivo, es. `ROMSAL000002`) | colonna `resources.code` additiva, **generata dal server** e mai accettata dal body (403, non scarto silenzioso): stessa forma per le persone (`ARMJUL000001`) e una forma DESCRITTIVA per i segnaposto (`ZZ - Dummy - <practice> - <ruolo>`), perché una riga che non è una persona non deve avere un codice che sembra una persona. Cercabile sia lato server (`/resources?q=`) sia nel filtro a schermo. L'`id` UUID resta la chiave; il codice è una chiave di ricerca umana | **PARI** — stessa convenzione, più la distinzione persona/segnaposto e l'unicità imposta da un indice parziale |
...
| 29 | **ServiceNow Requester Portal**: dal dummy si apre la demand di hiring/subco e si inserisce il **codice RES**; il codice del dummy viene riscritto includendo la RES (`RES0005555 - ZZ - Dummy - SAP - Associate PMO`) e il dummy passa da generico a specifico | seam **dichiarato non connesso**: `POST /integrations/demand/:resourceId` costruisce la requisizione che il portale riceverebbe, `PUT …/res-code` applica il numero RES e antepone il codice (`RES0005555 - ZZ - Dummy - …`), trasformando un dummy generico in specifico. Il RES-code è l\'unica azione di integrazione che SCRIVE, e ha una regola RBAC stretta propria (resource-manager, non finance). Nessuna rete | **PARZIALE** |
...
| 43 | **notifica email** ai responsabili alla creazione di dummy, subco e commessa Basket; la nota notifica l'approvatore | seam **dichiarato non connesso**: `POST /integrations/email/outbox` rende il messaggio per dummy, subco, commessa non fatturabile e approvazione in attesa. Nessun SMTP, nessun provider, nessuno stato `Sent` — i destinatari li risolve il chiamante, perché tradurre ruoli in indirizzi è una decisione di autorizzazione | **PARZIALE** |
...
| # | RPT | Delivery Control | Stato |
|---|---|---|---|
| 56 | alimentato da **5 sistemi**: Zucchetti (anagrafica risorse + organizzazione aziendale), Skill Matrix, PCP forecast, Capability; commesse da PCP/InforLN; skill da People Portal; hiring su ServiceNow | seam **dichiarato non connesso** a 7 adapter: ai 4 in uscita si aggiunge `inbound`, che dichiara i 6 sistemi a monte (Zucchetti, PCP, InforLN, People Portal, Skill Matrix, ServiceNow), normalizza i 3 di cui esiste una mappatura e risponde a «cosa CAMBIEREBBE questo payload» con un report create/update/unchanged/rejected per record. `applied: false` è nel TIPO, e non esiste una funzione di apply da chiamare per sbaglio | **PARZIALE** |
