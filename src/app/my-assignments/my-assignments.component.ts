          ...
          @if (viewMode() === 'week') {
                  ...
                  <tr class="text-sm text-ink-secondary hover:bg-surface-muted transition-colors group">
                    ...
                    @for (h of dailyHours(assignment.assignedHours); track $index) {
                      <td class="py-5 px-2 text-center">
                        <div class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-accent-tint text-accent-text ring-1 ring-accent font-bold font-mono tabular-nums shadow-sm group-hover:bg-[color-mix(in_oklch,var(--color-accent)_16%,var(--color-surface))] transition-colors">
                          {{ h }}
                        </div>
                      </td>
                    }
            ...
            <div class="grid grid-cols-7 gap-px bg-surface-muted rounded-xl overflow-hidden border border-line">
              ...
              @for (day of mockDays; track day.date) {
                <div class="bg-surface min-h-[100px] p-2 hover:bg-surface-muted transition-colors" [class.opacity-50]="!day.isCurrentMonth">
                  <div class="text-right text-xs font-medium text-ink-muted mb-2 font-mono tabular-nums">{{ day.date }}</div>
                  @if (day.isCurrentMonth && day.date % 2 !== 0 && myAssignments().length > 0) {
                    <div class="space-y-1">
                      <div class="text-[10px] font-medium bg-accent-tint text-accent-text ring-1 ring-accent px-2 py-1 rounded truncate" [title]="getRequestName(myAssignments()[0].requestId)">
                  ...
                  <div class="flex items-center gap-4">
                    @if (editingAssignmentId() === assignment.id) {
                      <div class="flex items-center gap-2">
                        <input type="number" [ngModel]="editHours()" (ngModelChange)="editHours.set($event)" class="command-input w-20 font-mono tabular-nums">
