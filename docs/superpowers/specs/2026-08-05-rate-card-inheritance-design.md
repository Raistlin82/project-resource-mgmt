...
`pickRateCard` (oggi `src/server.ts:1615-1620`, funzione privata) risolve la rate card di una risorsa in due passi: match **esatto** sul nome dell'organizzazione, altrimenti la card generica (senza organizzazione). Da quando l'albero D è arrivato (Capability > Practice > Competence, `resourceOrganizations.parentId`), quel match esatto ha una conseguenza che l'utente considera un difetto: **una card configurata su una capability non si applica alle practice o competence sottostanti**.
...
**`server.ts`:** rimuove la propria `RATE_BASE_CURRENCY` (riga 1591) e la propria `pickRateCard` (righe 1615-1620), importa entrambe da `rate-card.util.ts`, e passa un quarto argomento a `withEffectiveRates`/`pickRateCard`: `nodes`. `resolveResourceRates` (`src/server.ts:1646-1650`) aggiunge un `await repos.resourceOrganizations.list()` accanto ai due `await` che già fa (`repos.rateCards.list()`, `getHoursPerDay()`) — stessa forma, non una nuova categoria di cambiamento; nessuno dei 5 call site elencati sotto può già `await` (tutti dentro handler Express `async` o helper `async` chiamati da uno di essi):

1. `GET /resources` — `src/server.ts:1691`
2. `GET /resources/:id` — `src/server.ts:1696`
3. `POST /resources` (risposta dopo la creazione) — `src/server.ts:1807`
4. `PUT /resources/:id` (risposta dopo l'update) — `src/server.ts:2017`
5. `accruedTAndM()` — `src/server.ts:5004`, chiamata dal path di scrittura `/billing-plan-items` a `src/server.ts:5075`
...
**Riferimento verificato, e correzione di una citazione del brief:** l'implementazione di riferimento in questo codebase è `contract-details.ts::moneyFiguresState()` (`src/app/commercial/contract-details/contract-details.ts:1491-1495`) — l'**unico** computed `<'error' | 'loading' | 'ready'>` che esiste oggi nel repository (verificato via grep su tutto `src/app`). Il brief di questo blocco citava anche `what-if.ts::dataState()` come seconda implementazione di riferimento: **non esiste** — `src/app/forecast/what-if.ts` ha solo un `loading` booleano (`:420`, `dataRes.isLoading() || projectsRes.isLoading()`), nessun terzo stato `error`, nessuna funzione chiamata `dataState`. Questo documento usa **solo** `moneyFiguresState()` come riferimento; il gap di `what-if.ts` è preesistente e fuori scope qui.
...
**Per questo, il seed guadagna una riga nuova.** `src/db/seed.ts` aggiunge la risorsa `id: '7'`, `role: 'Developer'`, `organization: 'Backend'` (il nodo competence più profondo, `parentId` → `Platform` → `Engineering`), senza `costRate`/`billRate` propri (nessun override, per lasciare che la card faccia tutto il lavoro). Verificato a mano contro il seed live:
...
- **Mutazione `/rate-cards`:** `admin`, `delivery-executive`, `finance` (riga 654, verificata: `{ test: p => p.startsWith('/rate-cards'), roles: [...] }`).
- **Lettura `/rate-cards`:** `pm`, `resource-manager`, `delivery-executive`, `finance`, `admin` (riga 714, verificata).
...
- **Report d'impatto** (`scripts/rate-inheritance-impact.mjs`, §9): sul seed con la nuova riga `id:'7'`, esattamente una riga stampata con il delta letterale **cost +40,00 / bill +80,00 €/giorno** — eseguito prima del merge, come da nuova sezione nel deploy checklist (§9).
