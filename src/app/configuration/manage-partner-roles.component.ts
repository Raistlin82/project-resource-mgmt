    ...
    <div class="command-page max-w-5xl mx-auto space-y-8">
      <div class="flex items-center justify-between">
        <div>
          ...
          <h1 class="font-display text-2xl sm:text-3xl font-bold text-[var(--cc-ink)] tracking-tight">Manage Partner Roles</h1>
          <p class="mt-1 text-sm text-[var(--cc-muted)]">Maintain the project-partner relationship-role catalog.</p>
        </div>
        <button (click)="openForm()" class="command-button">
          <mat-icon class="text-sm">add</mat-icon> Add Partner Role
        </button>
      </div>
      ...
      <div class="command-card overflow-hidden">
        <div class="p-4 border-b border-[var(--cc-line)] flex gap-4 bg-[var(--cc-panel-muted)]">
          <div class="flex-1 relative">
            ...
            <input type="text" placeholder="Search partner roles..."
                   [ngModel]="search()" (ngModelChange)="search.set($event)"
                   aria-label="Search partner roles"
        ...
        <table class="command-data-table">
          ...
          <tbody>
            ...
            @if (filtered().length === 0) {
              <tr>
                <td colspan="2" class="text-center"><span class="text-[var(--cc-muted)]">No partner roles defined yet.</span></td>
              </tr>
            }
          </tbody>
        </table>
      ...
      @if (showForm()) {
        <div class="fixed inset-0 bg-scrim/40 backdrop-blur-sm flex items-center justify-center z-50 p-4"
             appModal ariaLabelledby="partnerRoleModalTitle" (dismiss)="closeForm()">
          <div class="command-card shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
            <div class="command-card-header">
              <h2 id="partnerRoleModalTitle" class="font-display text-xl font-bold text-[var(--cc-ink)]">{{ editingId() ? 'Edit Partner Role' : 'Add Partner Role' }}</h2>
            ...
            <form [formGroup]="form" (ngSubmit)="save()" class="p-6 space-y-4">
              <div>
                ...
                <input id="name" type="text" formControlName="name" class="command-input" placeholder="e.g. Subcontractor">
              </div>
              <div class="pt-4 flex justify-end gap-3">
                <button type="button" (click)="closeForm()" class="command-button secondary">Cancel</button>
                <button type="submit" [disabled]="!form.valid" class="command-button disabled:opacity-50 disabled:cursor-not-allowed">Save Partner Role</button>
              </div>
            </form>
          </div>
        </div>
      ...
      @if (deletingId()) {
        <div class="fixed inset-0 bg-scrim/40 backdrop-blur-sm flex items-center justify-center z-50 p-4"
             appModal ariaLabelledby="partnerRoleDeleteTitle" (dismiss)="cancelDelete()">
          <div class="command-card shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            <div class="p-6 text-center">
              ...
              <h3 id="partnerRoleDeleteTitle" class="font-display text-lg font-bold text-[var(--cc-ink)] mb-2">Delete Partner Role</h3>
              <p class="text-[var(--cc-muted)] text-sm">Are you sure you want to delete this partner role? This action cannot be undone.</p>
            </div>
...
export class ManagePartnerRolesComponent {
  ...
  save() {
    ...
    const payload: Partial<PartnerRole> = { name: this.form.getRawValue().name ?? '' };
    const id = this.editingId();
    const done = () => { this.itemsRes.reload(); this.closeForm(); this.notifications.show('Partner role saved.', 'success'); };
    if (id) {
      this.api.updatePartnerRole(id, payload).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(done);
    } else {
      this.api.createPartnerRole(payload).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(done);
    }
  }
  ...
  confirmDelete() {
    ...
    if (!id) return;
    this.api.deletePartnerRole(id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
      this.itemsRes.reload();
      this.deletingId.set(null);
      this.notifications.show('Partner role deleted.', 'success');
    });
  }
