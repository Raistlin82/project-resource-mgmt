...
| U14 | `forecast.util.ts:330-468` `capacityForecast` | orizzonte **settimanale** su `isActiveInMonth` + scalare `capacity` mai pro-ratato (commento `capacity.util.ts:69-73`) | **fuori scope, dichiarato** (§9). Il file dichiara già che sostituire il gate day-granular «*changes which weeks report supply at all (register P1-17/P1-18, still open), so it is that file's own change, not a drive-by*». Conseguenza da conoscere: la supply del forecast settimanale **continua a sovrastimare** per gli assenti |
| U15 | `forecast.util.ts:504-560` `skillGap` | supply per ruolo | stessa dipendenza di U14: un assente contato come supply **sottostima** un gap di skill. Fuori scope con la stessa motivazione, non per omissione |
...
- **Nessuna correzione della supply del forecast settimanale.** `capacityForecast` e `skillGap` (`forecast.util.ts:330,504`) continuano a **sovrastimare** la supply per gli assenti (U14/U15). La motivazione è del file stesso (`capacity.util.ts:69-73`, register P1-17/P1-18 aperto): passare al gate day-granular cambia quali settimane riportano supply, ed è un cambio di quel file, non un drive-by di questo. **Difetto residuo dichiarato, non omissione.**
