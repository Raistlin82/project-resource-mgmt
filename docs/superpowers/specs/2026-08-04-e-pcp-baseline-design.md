...
- Confronto: un mese in baseline assente dal piano (delta negativo, `outOfBaselineHorizon: false`, **`deltaPct` un numero finito e ben definito** — es. `-100.00%` quando il piano è arrivato a zero: la baseline non è zero in questo caso, quindi la regola "null solo se baseline = 0" di §4/§9 NON si applica, e il valore va mostrato normalmente, mai come trattino — è la variazione più severa che questo blocco esiste per segnalare, e nasconderla dietro il simbolo di "nessun dato" sarebbe la stessa confusione none-vs-unknown che questo progetto ha già pagato altrove); un mese nel piano assente dalla baseline (`outOfBaselineHorizon: true`, `deltaPct: null` — qui la baseline **è** zero, per assenza di riga); una baseline esplicitamente a zero dentro l'orizzonte (delta = planned intero, `deltaPct: null`, ma **senza** il flag di fuori-orizzonte — i due casi non vanno confusi); un ri-congelamento con due righe sullo stesso `(projectId, period)` — il confronto deve usare quella con `frozenAt` più recente, mai la prima trovata.
...
- **`cost_baselines` `'CB2'`**: `projectId: '1'`, `period: '2026-11'`, `amount: 500`, nessun `AssignmentDay` di progetto `'1'` in novembre nel seed. **Delta atteso: 0 − 500 = −500 EUR, `deltaPct = -100.00%`** (mostrato normalmente, **mai** un trattino: la baseline qui è 500, non zero, quindi la regola "null solo se baseline = 0" di §4 riga 139 / §9 non si applica — l'intera baseline è evaporata, ed è esattamente il segnale più forte che questo confronto deve rendere visibile) — il caso "baseline congelata, piano che non la copre più" (descoping), senza bisogno di alcuna nuova prenotazione.
...
**Smoke** (`scripts/cost-baseline-impact.mjs`, dependency-free, modellato su `scripts/negotiated-rate-impact.mjs`): congela una baseline sul progetto fixture via `POST`, verifica che una seconda lettura restituisca esattamente le righe scritte (nessuna deriva), ricalcola il confronto e verifica il totale contro i valori attesi sopra (+120 EUR/+20.00% su ottobre, −500 EUR/−100.00% su novembre — **mai** un trattino: la baseline di novembre è 500, non zero), verifica il rifiuto RBAC 403 per un ruolo `pm` su `POST /cost-baselines`. **Dichiarazione esplicita, sulla scia della lezione della spec sorella:** una tabella `cost_baselines` vuota che produce un confronto tutto a zero è vera per costruzione, non evidenza di correttezza — il gate reale è il caso non-nullo sopra, con un delta calcolabile a mano.
