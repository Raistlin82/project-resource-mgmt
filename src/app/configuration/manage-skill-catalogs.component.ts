        ...
        <div class="p-6 sm:p-8 border-b border-slate-200/60 bg-slate-50/80 backdrop-blur-sm">
          <form [formGroup]="catalogForm" (ngSubmit)="onSubmit()" class="space-y-6 max-w-md">
            <div>
              ...
              <input id="catalogName" type="text" formControlName="name" class="w-full px-4 py-3 rounded-xl border border-slate-200/60 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none bg-white shadow-inner font-bold text-slate-900 transition-all">
            </div>
            <div>
              ...
              <textarea id="catalogDescription" formControlName="description" rows="3" class="w-full px-4 py-3 rounded-xl border border-slate-200/60 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none bg-white shadow-inner font-medium text-slate-700 transition-all"></textarea>
            </div>
            <div class="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" [disabled]="catalogForm.invalid" class="px-6 py-2.5 text-sm font-bold text-white bg-indigo-600 border border-transparent rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">Save</button>
            </div>
          </form>
        </div>
      }
            ...
            <tbody class="text-sm">
              @for (catalog of catalogs(); track catalog.id) {
                <tr class="border-b border-slate-100 hover:bg-slate-50/80 transition-colors group">
                  <td class="py-5 text-slate-900 font-bold text-base group-hover:text-indigo-700 transition-colors">{{ catalog.name }}</td>
                  <td class="py-5 text-slate-600 font-medium">{{ catalog.description }}</td>
                  <td class="py-5 text-center">
                    <span class="inline-flex items-center justify-center w-8 h-8 text-xs font-bold text-indigo-700 bg-indigo-50 rounded-xl border border-indigo-100">{{ catalog.skills.length }}</span>
                  </td>
                  <td class="py-5 text-right">
                    <button (click)="deleteCatalog(catalog.id)" class="w-10 h-10 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all inline-flex items-center justify-center shadow-sm" title="Delete">
...
export class ManageSkillCatalogsComponent implements OnInit {
  ...
  loadCatalogs() {
    this.api.getSkillCatalogs().subscribe(res => this.catalogs.set(res));
  }
