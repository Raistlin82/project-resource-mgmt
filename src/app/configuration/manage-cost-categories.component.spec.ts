...
const COST_CATEGORIES: CostCategory[] = [{ id: 'CAT1', name: 'Travel & Expenses' }];

function setup(items: CostCategory[] = COST_CATEGORIES) {
  const apiStub = {
    getCostCategories: vi.fn(() => of(items)),
    createCostCategory: vi.fn(() => of({} as CostCategory)),
    updateCostCategory: vi.fn(() => of({} as CostCategory)),
    deleteCostCategory: vi.fn(() => of(undefined as unknown as void)),
  } as unknown as ApiService;
  const authStub = { authReady: signal(true), isAuthenticated: signal(true) } as unknown as AuthService;

  TestBed.configureTestingModule({
    imports: [ManageCostCategoriesComponent],
    providers: [
  ...
  const fixture = TestBed.createComponent(ManageCostCategoriesComponent);
  return { fixture };
}
...
describe('ManageCostCategoriesComponent form overlay — STRUCTURAL contract only (jsdom performs no layout)', () => {
  ...
  it('declares its own scroller, a top anchor and a bounded panel whose body scrolls', async () => {
    ...
    // header goes above y=0 and the "Save Cost Category" footer below the fold — and a
    ...
    const overlay = host.querySelector<HTMLElement>('[data-test="cost-category-form-overlay"]')!;
    const panel = host.querySelector<HTMLElement>('[data-test="cost-category-form-panel"]')!;
  ...
  it('keeps the submit control inside the form element, so Enter still submits', async () => {
    ...
    const form = (fixture.nativeElement as HTMLElement).querySelector<HTMLFormElement>('[data-test="cost-category-form-panel"] form')!;
    const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]');
    expect(submit).not.toBeNull();
    expect(submit!.textContent?.trim()).toBe('Save Cost Category');
  });
  ...
  it('rejects a plain centred overlay — the negative control that keeps the predicate honest', async () => {
    ...
    fixture.componentInstance.deleteItem('CAT1');
    fixture.detectChanges();

    const control = (fixture.nativeElement as HTMLElement).querySelector<HTMLElement>('[data-test="cost-category-delete-overlay"]')!;
